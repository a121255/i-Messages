...
import com.test.imessagesbackend.common.config.jwt.JwtProperties;
...
public class JwtProviderTest {
    ...
    void generateToken() {
        ...
        // then
        Long userId = Jwts.parser() // 토큰...에서 유저이이디 꺼낸다
    ...
    void validToken_invalidToken(){
        ...
        String token = JwtFactory.builder()
                .expiration(new Date(new Date().getTime() // 현재 시간에서 7일을 뺀 시간으로 토큰 생성
                        - Duration.ofDays(7).toMillis())).build().createToken(jwtProperties);
    ...
    void getAuthentication() {
        // given
        String userEmail = "user@mdmd.com";
        String token = JwtFactory.builder()
                .subject(userEmail)
                .build()
